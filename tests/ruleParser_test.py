...
class RuleParser(object): 
  ...
  def parse(self, input:list, id:str) -> Union[list, str]: 
    parsed = list()
            ...
            if len(tmp) > 1: 
              for sub in tmp: 
                if sub[:-1] not in self.ids: 
                  parsed.append('(?P<%s>%s)%s' % (id, sub[:-1], sub[-1]))
                  continue          # else:
                parsed.append(sub)  
            else: 
              if arg[:-1] not in self.ids: 
                parsed.append('(?P<%s>%s)%s' % (id, arg[:-1], arg[-1]))
                continue          # else 
              parsed.append(arg)
          continue                # else:
        parsed.append(tuple(tmp))
        continue                 
    return tuple(parsed) if len(parsed) > 1 else parsed[0] 
