...
argparser = argparse.ArgumentParser(
    description="Scan a json file for grammatical accuracy.")
    ...
    for id in self._rules:
      ...
      for rule in self._rules[id]:
        ...
        if rule == '<capture>':
          ...
          self._captures[id] = self._captures[id].replace(
              '\(', '').replace('\)', '')
...
class _RuleParser(object):
  ...
  def parse_rule(self, rules):
    ...
    for rule in rules:
      ...
      if captured:
        # multi - rule capture
        tmp.append(self.tokenizer((rule_id, rule_st), rule_ids, captured[0]))
      else:
        tmp.append(self.tokenizer((rule_id, rule_st), rule_ids)
                   )              # single - rule capture
  ...
  def tokenizer(self, rule, ids, _capture=None):
    ...
    rule_sts = [(rule[0], rule[1])] if not _capture else [
    ...
    if __depth__ > 0:
      ...
      if _capture:
        print(
            "\x1b[1m[\x1b[33m!\x1b[0m]\x1b[1m\t      \x1b[3mCapture Group:\x1b[0m", _capture)
